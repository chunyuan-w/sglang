...
TORCH_LIBRARY_FRAGMENT(sgl_kernel, m) {
  ...
  m.def(
      "topk_sigmoid_bias_cpu(Tensor hidden_states, Tensor gating_output, int topk, bool renormalize, "
      "Tensor? correction_bias) -> (Tensor, Tensor)");
  m.impl("topk_sigmoid_bias_cpu", torch::kCPU, &topk_sigmoid_bias_cpu);
  m.def(
