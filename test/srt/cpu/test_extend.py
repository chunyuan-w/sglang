...
class TestExtendAttention(CustomTestCase):
    ...
    def _test_extend_attention_fixed_lens(
        ...
        # Check for NaN first — a negative last_col bug produces NaN in output
        self.assertFalse(
            torch.isnan(o_extend).any().item(),
            "NaN detected in extend_attention output (possible causal-mask out-of-bounds write)",
        )
        torch.testing.assert_close(o_ref, o_extend, atol=1e-2, rtol=1e-2)
    ...
    def test_extend_attention_large_seq_causal_mask(self):
        # Regression test for causal-mask out-of-bounds write (negative last_col).
        #
        # With max_len_extend > 4096, the kernel selects BLOCK_M=512, BLOCK_N=768.
        # Because 2*BLOCK_M=1024 > BLOCK_N=768, the stage-2 key loop can produce
        # a second n-block at n=768 when m=512 (mb=1).  For that block n > m, so
        # last_col = m + row - n < 0 for rows 0..255.  The original code did:
        #   fill_stub(row_ptr + last_col + 1, -inf, n_size - last_col - 1)
        # which wrote ~511 floats starting 255 positions *before* row_ptr,
        # corrupting adjacent thread buffers and producing NaN in the output.
        #
        # The fix clamps last_col to -1 so the fill always starts at row_ptr[0].
        # This test uses extend_len=5000 (no prefix) to deterministically hit
        # the first trigger point: m=512, n=768, last_col=-256.
